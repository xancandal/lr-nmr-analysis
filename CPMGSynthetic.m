sampling_rate = 1e-4;
nbr_data_points = 2^10; noise_level = 2e-1;
...
%assert( size(t2Values) == size(amplitudes) );
idy = 1./t2Values;
...
axis([0.5*min(t2Values) 2*max(t2Values) 0.8*min(amplitudes) 1.2*max(amplitudes)]); title('T2 Components and their Amplitudes');