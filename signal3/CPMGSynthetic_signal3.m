function [data,snr] = CPMGSynthetic_signal3( t2Values, amplitudes, sampling_period, nbr_data_points, noise_level, n, deltaT, draw, wait )

% CPMGSynthetic( t2Values, amplitudes, sampling_period, nbr_data_points, snr );
...
ti = t(length(t)-length(t)*temp:end);
yi = y(length(y)-length(y)*temp:end);
